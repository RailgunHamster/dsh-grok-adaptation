...
function responsesPath(url) {
  if (!url) return false;
  const path = url.pathname.replace(/\/+$/, "");
  return path === "/v1/responses" || path === "/responses";
}

function hostnameAllowed(url, hostnames) {
  if (!url) return false;
  if (!Array.isArray(hostnames) || hostnames.length === 0) return true;
...
function apply(ctx, config = {}) {
  ...
  const hostnames = Array.isArray(config.hostnames)
    ? config.hostnames.filter((value) => typeof value === "string" && value.length > 0)
    : [];
  ...
  ctx.effect(() => {
    ...
    const wrappedFetch = async (input, init = {}) => {
      ...
      if (!responsesPath(url) || !hostnameAllowed(url, hostnames)) return originalFetch(input, init);
