...
function apply(ctx, config = {}) {
  ...
  ctx.effect(() => {
    ...
    const wrappedFetch = async (input, init) => {
      ...
      if (!marked || url?.hostname !== "api.niumacode.cc" || url?.pathname.replace(/\/+$/, "") !== "/v1/responses") {
        return originalFetch(input, init);
      }
      return bridgeFetch(input, init, { websocketBaseURL, websocketBeta, handshakeTimeoutMs });
    };
