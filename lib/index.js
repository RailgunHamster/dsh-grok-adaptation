...
function apply(ctx, config = {}) {
  ...
  const log = (message) => {
    if (debug && typeof console !== "undefined" && console.log) console.log(`[dsh-niuma-responses-ws] ${message}`);
  };
  ...
  ctx.effect(() => {
    ...
    const wrappedFetch = async (input, init = {}) => {
      ...
      if (!marked && !isAutoModel) return forwardFetch(originalFetch, input, init, url, headers, rawBody);
