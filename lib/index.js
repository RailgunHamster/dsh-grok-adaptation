...
async function bridgeFetch(input, init, config, rawBody) {
    ...
    start(controller) {
      ...
      const onMessage = (data) => {
        ...
        try {
          ...
          if (event.type === "response.completed" || event.type === "response.failed" || event.type === "error") closeNormally();
        } catch {}
      };
      socket.on("message", onMessage);
      socket.on("error", closeWithError);
      socket.on("close", (code, reason) => {
        if (!closed) closeWithError(new Error(`NiuMa WebSocket closed ${code}: ${reason.toString()}`));
      });
...
function apply(ctx, config = {}) {
  ...
  ctx.effect(() => {
    ...
    const wrappedFetch = async (input, init) => {
      ...
      if (marked) return bridgeFetch(input, init, { websocketBaseURL, websocketBeta, handshakeTimeoutMs });
      ...
      if (rawBody === undefined) return originalFetch(input, init);
      let body;
      try { body = JSON.parse(rawBody); } catch { return originalFetch(input, init); }
      ...
      return bridgeFetch(input, { ...init, body: rawBody }, { websocketBaseURL, websocketBeta, handshakeTimeoutMs }, rawBody);
    };
